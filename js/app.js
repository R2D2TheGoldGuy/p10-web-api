...
function updateGallery() {
  // Clear the gallery
  $("main.spotify-data").fadeOut(300, function(){
    $("main.spotify-data").empty();
    if (albumList.length > 0) {
      // Iterate through each result and append to gallery
      $.each(albumList, function(index, item) {
        var albumHTML = buildAlbum(item, index);
        $("main.spotify-data").append(albumHTML);
      });
      $("main.spotify-data").fadeIn(300);
    }
...
$("document").ready(function(){
  ...
  $(".spotify-lightbox").click(function(){
    $(this).fadeOut();
    $(".info").fadeOut(1);
  });
  ...
  $(document).keydown(function(event){
    if ($(".spotify-lightbox").is(":visible") && event.which === 37)
    {
      updatePrev();
    }
    else if ($(".spotify-lightbox").is(":visible") && event.which === 39)
    {
      updateNext();
    }
    else if ($(".spotify-lightbox").is(":visible") && event.which === 27)
    {
      $(".spotify-lightbox").fadeOut();
      $(".info").fadeOut(1);
    }
  });
